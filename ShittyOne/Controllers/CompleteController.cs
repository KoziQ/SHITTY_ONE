...
public class CompleteController : ControllerBase
{
    ...
    {
        var survey = await _dbContext.Surveys.AsNoTracking().Include(s => s.Questions.OrderBy(q => q.Title))
    ...
    /// <returns></returns>
    [HttpPost("{questionId}")]
    public async Task<IActionResult> CompleteQuestion(Guid questionId, Guid surveyId, [FromBody] PostUserAnswer model,
        Guid? sessionId = null)
    {
        ...
        var question = await _dbContext.Set<SurveyQuestion>()
            .FirstOrDefaultAsync(q =>
                q.Id == questionId && q.SurveyId == surveyId && (q.Users.Any(u => u.Id == user.Id) ||
        ...
        if (sessionId != null)
        {
            session = await _dbContext.UserSessions.Include(s => s.Answers)
                .FirstOrDefaultAsync(s => s.Id == sessionId && s.SurveyId == surveyId);

            if (session == null || session.End != null) return NotFound();
        }
        else
        {
        ...
        var questions = _dbContext.Set<SurveyQuestion>().Where(q => q.Users.Any(u => u.Id.ToString() == User.GetId()))
            .Count();
        //Check if session was completed
        if (session.Answers.GroupBy(a => a.QuestionId).Count() == questions) session.End = DateTime.Now;
