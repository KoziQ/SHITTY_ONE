...
public class GroupsController : ControllerBase
{
    ...
    public IActionResult Select(int page = 1, int size = 10, string? search = null)
    {
        ...
        if (!string.IsNullOrEmpty(search)) groups = groups.Where(g => EF.Functions.Like(g.Title, $"%{search}%"));

        var total = groups.Count();

        return Ok(new SelectModel<GroupModel>
        {
            Data = groups.OrderBy(g => g.Title).Skip((page - 1) * size).Take(size)
                .Select(g => _mapper.Map<GroupModel>(g)).ToList(),
    ...
    public IActionResult SelectUsers(int page = 1, int size = 10, string? search = null)
    {
        ...
        if (!string.IsNullOrEmpty(search))
            users = users.Where(g =>
                EF.Functions.Like(g.UserName, $"%{search}%") || EF.Functions.Like(g.Email, $"%{search}%"));

        var total = users.Count();

        return Ok(new SelectModel<UserModel>
        {
            Data = users.OrderBy(g => g.UserName).Skip((page - 1) * size).Take(size)
                .Select(g => _mapper.Map<UserModel>(g)).ToList(),
            More = total > page * size
        });
    }
