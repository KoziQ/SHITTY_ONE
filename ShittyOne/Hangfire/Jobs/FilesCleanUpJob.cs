...
public class FilesCleanUpJob : IRecurringJob
{
    ...
    public async Task Execute()
    {
        ...
        foreach (var file in Directory.GetFiles(
                     Path.Combine(_hostEnvironment.WebRootPath, "uploads".TrimStart(Path.AltDirectorySeparatorChar)),
                     "*", SearchOption.AllDirectories))
        {
            ...
            if (!await _dbContext.Files.AnyAsync(f =>
                    filename.Replace(_hostEnvironment.WebRootPath, "").Replace("\\", "/") == f.SubDir))
                try
                {
    ...
    private async Task<List<File>> GetJunkFiles()
    {
        ...
        foreach (var type in entityTypes)
            filesQueryBuilder.AppendLine(
                $" AND Id NOT IN(SELECT ISNULL(FileId,0) FROM [dbo].{type.GetTableName()} WHERE FileId IS NOT NULL)");
        ...
        foreach (var type in entityTypes)
            filesQueryBuilder.AppendLine(
                $" AND Id NOT IN(SELECT ISNULL(IconId, 0) FROM [dbo].{type.GetTableName()} WHERE IconId IS NOT NULL)");
        ...
        foreach (var type in entityTypes)
            filesQueryBuilder.AppendLine(
                $"AND NOT EXISTS(SELECT * FROM [dbo].{type.GetTableName()} WHERE JsonContent LIKE '%' + {nameof(File.SubDir)} + '%')");

        return await _dbContext.Files.FromSql(FormattableStringFactory.Create(filesQueryBuilder.ToString()))
            .ToListAsync();
    }
}