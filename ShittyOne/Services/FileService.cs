...
{
    public Task<File?> AddFile(IFormFile file);
...
public class FileService : IFileService
{
    ...
    {
        if (GetContentType(file.ContentType) == null) throw new FormatException("unsupported media type");
        ...
        using (var sr = new StreamContent(file.OpenReadStream()))
        {
            ...
            var workingFolder = Path.Combine(_hostEnviroment.WebRootPath,
                directory.TrimStart(Path.AltDirectorySeparatorChar));

            if (!Directory.Exists(workingFolder)) Directory.CreateDirectory(workingFolder);
            var fileGuid = Guid.NewGuid().ToString();

            await System.IO.File.WriteAllBytesAsync(
                Path.Combine(workingFolder, $"{fileGuid}{GetContentType(file.ContentType)}"), content);
    ...
    public async Task<bool> RemoveFile(Guid fileId)
    {
        ...
        var filePath = Path.Combine(_hostEnviroment.WebRootPath, file.SubDir.TrimStart(Path.AltDirectorySeparatorChar));
